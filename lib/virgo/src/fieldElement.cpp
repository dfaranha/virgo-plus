...
namespace virgo {
    ...
    fieldElement fieldElement::operator-(const fieldElement &other) const {
        ...
        fieldElement ret;
		for (size_t i = 0; i < 2; i++) {
			auto tmp_r = mod - other.elem[i]; //tmp_r == -b.real is true in this prime field
			ret.elem[i] = elem[i] + tmp_r;
			if (ret.elem[i] >= mod)
				ret.elem[i] -= mod;
		}
    ...
    fieldElement fieldElement::maxWithZero(const fieldElement &a, const fieldElement &b) {
        ...
        return fieldElement(a.isNegative() ? b : b.isNegative() ? a : std::max(a.elem[0], b.elem[0]));
    }
